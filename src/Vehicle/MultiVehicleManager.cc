...
void MultiVehicleManager::_setActiveVehicle(Vehicle *vehicle)
{
    if (vehicle != _activeVehicle) {
        ...
        // Exponer la IP del ordenador de abordo (192.168.X.163) a QML, p.ej. para
        // los backups. El control del stream de vídeo (start/stop) ya NO se hace
        // aquí: lo dispara LinkManager al conectar/desconectar el comm link, así
        // no hace falta tener un vehículo activo para que salga el "start".
